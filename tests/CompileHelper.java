...
public class CompileHelper {
	...
	public static Program buildProjectFromClassPathFile(File classpath)
			throws Exception {
		...
		NodeList classpathentry_nodes = doc
		...
		for (int i = 0; classpathentry_nodes.item(i) != null; i++) {
			...
			if (n.getAttributes().getNamedItem("kind") == null)
				continue;
			String kind_value = n.getAttributes().getNamedItem("kind")
					.getNodeValue();
			if (n.getAttributes().getNamedItem("path") == null)
				continue;
			String path_value = n.getAttributes().getNamedItem("path")
					.getNodeValue();
			if (kind_value.equals("src")) {
				if (!path_value.startsWith(File.separator)
						&& !path_value.contains(":"))
					path_value = classpath.getParentFile().getPath()
							+ File.separator + path_value;
	...
	public static Program process(String[] arglist) {
		Frontend f = new Frontend() {
			protected void processWarnings(Collection errors,
