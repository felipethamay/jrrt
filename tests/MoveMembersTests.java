...
public class MoveMembersTests extends TestCase {
	...
	private void testSucc(String sourceType, String[] members,
			String targetType, Program in, Program out) {
		assertNotNull(in);
		assertNotNull(out);
	...
	private void testFail(String sourceType, String[] members,
			String targetType, Program in) {
		assertNotNull(in);
		TypeDecl source = in.findType(sourceType);
		assertNotNull(source);
		TypeDecl target = in.findType(targetType);
	...
	public void test1() {
		testSucc("A", new String[] { "f" }, "B",
				Program.fromClasses("class A { int f; }", "class B { }"),
				Program.fromClasses("class A { }", "class B { int f; }"));
	}

	public void test2() {
		testSucc("A", new String[] { "f" }, "B", Program.fromClasses(
				"class A { final int f = 23; }", "class B { }"),
				Program.fromClasses("class A { }",
						"class B { final int f = 23; }"));
	}

	public void test3() {
		testFail("A", new String[] { "f" }, "B", Program.fromClasses(
				"class A { final int f; { f = 23; } }", "class B { }"));
	}

	public void test4() {
		testSucc("A", new String[] { "f" }, "B", Program.fromClasses(
				"class A { class C { } C f; }", "class B { }"),
				Program.fromClasses("class A { class C { } }",
						"class B { A.C f; }"));
	}
