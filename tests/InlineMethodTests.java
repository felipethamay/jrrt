...
public class InlineMethodTests extends TestCase {
	...
	private MethodAccess findAccess(Program in) {
		...
		Stmt s = l.unlabel();
		assertTrue("not found", s instanceof ExprStmt
				&& ((ExprStmt) s).getExpr().isMethodAccess());
	...
	public void test0() {
		testSucc(Program.fromBodyDecls("int m() { return ((n())); }",
				"int n() { return 23; }"), Program.fromBodyDecls(
				"int m() { return 23; }", "int n() { return 23; }"));
	}
	...
	public void test10() {
		testSucc(Program.fromCompilationUnits(new RawCU("A.java", "class A {"
				...
				+ "    void n(int i) {" + "        if(i == 42)"
				+ "            return;"
				+ "        System.out.println(\"here; i == \"+i);" + "    }"
				+ "}")), Program.fromCompilationUnits(new RawCU("A.java",
				"class A {" + "  void m() {" + "    int i = 23;" + "    l:{"
						+ "      int i0 = 23;" + "      if(i0 == 42) "
						+ "        break l;"
	...
	public void test15() {
		testSucc(Program.fromCompilationUnits(new RawCU("A.java", "class A {"
				...
				+ "        inline: n(23);"
				+ "        System.out.println(\"back\");" + "    }"
				+ "    void n(int i) {" + "        l : while(i == 2) { }"
				+ "        if(i == 42)" + "            return;"
				+ "        System.out.println(\"here; i == \"+i);" + "    }"
				+ "}")), Program.fromCompilationUnits(new RawCU("A.java",
				"class A {" + "  void m() {" + "    int i = 23;" + "    l0:{"
						+ "      int i0 = 23;" + "      l:"
						+ "        while(i0 == 2){" + "        }"
						+ "      if(i0 == 42) " + "        break l0;"
						+ "      System.out.println(\"here; i == \" + i0);"
	...
	public void test17() {
		testSucc(Program.fromBodyDecls("int sum(int... xs) {"
				+ "  int sum = 0;" + "  for(int x : xs)" + "    sum += x;"
				+ "  return sum;" + "}", "{ int sum;"
				+ "  sum = ((sum(23, 42, 56))); }"),
				Program.fromBodyDecls("int sum(int... xs) {" + "  int sum = 0;"
						+ "  for(int x : xs)" + "    sum += x;"
						+ "  return sum;" + "}" + "{ int sum;"
						+ "  int[] xs = new int[] { 23, 42, 56 };"
						+ "  int sum0 = 0;" + "  for(int x : xs)"
						+ "    sum0 += x;" + "  sum = sum0; }"));
	}
	...
	public void test19() {
		testSucc(Program.fromClasses("class A {" + "  public long m() {"
				...
				+ "  public long m() {" + "    return ((super.m()));" + "  }"
				+ "  public long test() {" + "    return m();" + "  }" + "}"),
				Program.fromClasses("class A {" + "  public long m() {"
						+ "    return k();" + "  }" + "  public long k() {"
						+ "    return 10;" + "  }" + "}", "class B extends A {"
	...
	public void test20() {
		testSucc(Program.fromClasses("class A {"
				...
				+ "  int k() { return 23; }" + "}", "class B extends A {"
				+ "  { inline: m(); }" + "}"), Program.fromClasses("class A {"
				+ "  int m() { return A.this.k(); }"
				+ "  int k() { return 23; }" + "}", "class B extends A {"
				+ "  { k(); }" + "}"));
	}
