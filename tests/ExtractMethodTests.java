...
public class ExtractMethodTests extends TestCase {
	...
	public void testSucc(Program in, Program out) {
		...
		assertNotSame(end, -1);
		try {
			block.doExtractMethod(ASTNode.VIS_PROTECTED, "extracted", start,
					end);
	...
	public void testSucc(String className, String methodName, int begin,
			int end, String newMethodName, int visibility, Program in,
			Program out) {
		assertNotNull(in);
		String originalProgram = in.toString();
		if (AllTests.TEST_UNDO)
		...
		if (m != null) {
			...
			b = ((ConstructorDecl) A.constructors().iterator().next())
					.getBlock();
		}
		try {
	...
	public void testSucc(String className, String methodName, int begin,
			int end, String newMethodName, Program in, Program out) {
		testSucc(className, methodName, begin, end, newMethodName,
				ASTNode.VIS_PRIVATE, in, out);
	}
	...
	public void testFail(String className, String methodName, int begin,
			int end, String newMethodName, int visibility, Program in) {
		assertNotNull(in);
		String originalProgram = in.toString();
		if (AllTests.TEST_UNDO)
	...
	public void testFail(String className, String methodName, int begin,
			int end, String newMethodName, Program in) {
		testFail(className, methodName, begin, end, newMethodName,
				ASTNode.VIS_PRIVATE, in);
	}
	...
	public void test16() {
		testFail("B", "m", 0, 0, "n", ASTNode.VIS_PACKAGE,
				Program.fromCompilationUnits(new RawCU("A.java", "class B {\n"
	...
	public void test43() {
		testSucc("A", "m", 1, 1, "extracted", ASTNode.VIS_PROTECTED,
				Program.fromCompilationUnits(new RawCU("A.java",
	...
	public void test45() {
		testSucc("A", "m", 0, 0, "extracted", ASTNode.VIS_PROTECTED,
				Program.fromCompilationUnits(new RawCU("A.java", "class A {\n"
						+ "    int m() {\n" + "	     do {\n"
						+ "	         return 42;\n" + "	     } while(false);\n"
						+ "    }\n" + "}")),
