...
public class AddParameterTests extends TestCase {
	private void testSucc(String hosttp, String sig, int i, String parmtp,
		...
		assertNotNull(md);
		try {
			md.doAddParameter(new ParameterDeclaration(new TypeAccess(parmtp),
					parmname), i, val, false);
	...
	public void test1() {
		testSucc("p.A", "k()", 0, "int", "i", new IntegerLiteral(0),
				Program.fromCompilationUnits(new RawCU("A.java", "package p;"
						...
						new RawCU("B.java", "package q;" + "import p.*;"
	...
	public void test2() {
		testSucc("A", "k()", 0, "int", "i", new IntegerLiteral(0),
				Program.fromClasses("class A {" + "  long k() { return 0; }"
						+ "  public long m() {" + "    return A.this.k();"
						+ "  }" + "}", "class B extends A {"
						+ "  protected long k() { return 2; }"
						+ "  public long test() { return m(); }" + "}"),
				Program.fromClasses("class A {"
						+ "  long k(int i) { return 0; }"
						+ "  public long m() {" + "    return A.this.k(0);"
						+ "  }" + "}", "class B extends A {"
