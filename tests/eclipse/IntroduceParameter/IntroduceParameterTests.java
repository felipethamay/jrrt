...
public class IntroduceParameterTests extends TestCase {
	...
	static class IntroduceParameterData {
		...
		public IntroduceParameterData(String name, int startLine,
	...
	private IntroduceParameterData getData(String fileName) throws Exception {
		...
		String[] fields = firstLine.substring("//selection: ".length()).split(
				",\\s*");
		assertTrue(fields.length == 4);
		String secondLine = br.readLine();
		String[] names;
		if (secondLine.matches("^//name: .*$")) {
			names = secondLine.substring("//name: ".length()).split(
					"\\s*->\\s*");
		} else {
			names = new String[] { "", "arg" };
		}
		assertTrue(names.length == 2);
		return new IntroduceParameterData(names[1],
	...
	private void perform(boolean succeed) throws Exception {
		String in_name = "tests/eclipse/IntroduceParameter/"
				+ (succeed ? "simple" : "invalid") + "/"
				+ getName().substring(succeed ? 11 : 12) + ".java";
		Program in = CompileHelper.compile(in_name);
		Program out = succeed ? CompileHelper
				.compile("tests/eclipse/IntroduceParameter/simple/out/"
						+ getName().substring(11) + ".java") : null;
		assertNotNull(in);
		...
		Expr e = ExtractTempTests.findExpr(in, data.startLine,
				data.startColumn, data.endLine, data.endColumn);
		if (!succeed && e == null)
			return;
		assertNotNull(e);
		try {
	...
	/*
	 * disabled: does not compile public void testInvalid_NoMethodBinding()
	 * throws Exception { performInvalidSelection(); }
	 */
	...
	/*
	 * disabled: conservative data flow public void testSimple_Capture() throws
	 * Exception { performOK(); }
	 */

	/*
	 * disabled: clone detection public void testSimple_ConstantExpression1()
	 * throws Exception { performOK(); }
	 */

	/*
	 * disabled: conservative data flow public void
	 * testSimple_ConstantExpression2() throws Exception { performOK(); }
	 */
	...
	/*
	 * disabled: conservative data flow public void
	 * testSimple_NewInstanceImport() throws Exception { performOK(); }
	 */
	...
	/*
	 * disabled: conservative data flow public void testSimple_Wildcard1()
	 * throws Exception { performOK(); }
	 */

	/*
	 * disabled: conservative data flow public void testSimple_Wildcard2()
	 * throws Exception { performOK(); }
	 */
}
