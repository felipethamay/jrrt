...
public class IntroduceParameterObjectTests extends TestCase {
	...
	private void runRefactoring(boolean expectError, boolean toplevel)
			throws Exception {
		runRefactoring(null, expectError, toplevel);
	}

	private void runRefactoring(String[] parms, boolean expectError,
			boolean toplevel) throws Exception {
		runRefactoring(parms, "FooParameter", "parameterObject", expectError,
				toplevel);
	}

	private void runRefactoring(String className, String parmName,
			boolean expectError, boolean toplevel) throws Exception {
		runRefactoring(null, className, parmName, expectError, toplevel);
	}

	private void runRefactoring(String[] parms, String className,
			String parmName, boolean expectError, boolean toplevel)
			throws Exception {
		Program in = CompileHelper
				.compileAllJavaFilesUnder("tests/eclipse/IntroduceParameterObject/"
						+ getName() + "/in");
		Program out = expectError ? null
				: CompileHelper
						.compileAllJavaFilesUnder("tests/eclipse/IntroduceParameterObject/"
								+ getName() + "/out");
		assertNotNull(in);
		...
		try {
			...
			foo.doIntroduceParameterObject(
					parms == null ? null : Arrays.asList(parms), className,
					parmName, toplevel);
	...
	/*
	 * disabled: delegation creation not supported yet public void
	 * testDelegateCreation() throws Exception { runRefactoring(false, false); }
	 * 
	...
	public void testImportAddTopLevel() throws Exception {
		runRefactoring("TestImportAddTopLevelParameter", "parameterObject",
				false, true);
	}
	...
	/*
	 * disabled: reordering not supported yet public void
	 * testRecursiveReordered() throws Exception { runRefactoring(false, false);
	 * }
	 */
	...
	/*
	 * disabled: reordering not supported yet public void
	 * testRecursiveSimpleReordered() throws Exception { runRefactoring(false,
	 * false); }
	 */

	/*
	 * disabled: delegation creation not supported yet public void
	 * testReorderGetter() throws Exception{ runRefactoring(false, false); }
	 */
	...
	/*
	 * disabled: reordering not supported yet public void testVarArgsReordered()
	 * throws Exception{ runRefactoring(false, false); }
	 */
}
