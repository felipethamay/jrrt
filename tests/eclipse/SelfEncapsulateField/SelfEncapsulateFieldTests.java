...
public class SelfEncapsulateFieldTests extends TestCase {
	...
	protected void performTest(String id, String folder, String fieldName)
			throws Exception {
		Program in = CompileHelper.compile(getResourceLocation() + "/" + folder
				+ "_in/" + adaptName(id));
		Program out = CompileHelper.compile(getResourceLocation() + "/"
				+ folder + "_out/" + adaptName(id));
		assertNotNull(in);
	...
	protected void performInvalidTest(String id, String folder, String fieldName)
			throws Exception {
		Program in = CompileHelper.compile(getResourceLocation() + "/" + folder
				+ "/" + adaptName(id));
		assertNotNull(in);
	...
	private void performStaticImportTest(String name, String referenceName)
			throws Exception {
		Program in = CompileHelper.compile(getResourceLocation()
				+ "/static_in/" + name + ".java", getResourceLocation()
				+ "/static_ref_in/" + referenceName + ".java");
		Program out = CompileHelper.compile(getResourceLocation()
				+ "/static_out/" + name + ".java", getResourceLocation()
				+ "/static_ref_out/" + referenceName + ".java");
		assertNotNull(in);
		String originalProgram = in.toString();
		if (AllTests.TEST_UNDO)
			Program.startRecordingASTChangesAndFlush();
	...
	public void testStaticImportRead() throws Exception {
		performStaticImportTest("TestStaticImportRead",
				"StaticImportReadReference");
	}

	public void testStaticImportWrite() throws Exception {
		performStaticImportTest("TestStaticImportWrite",
				"StaticImportWriteReference");
	}

	public void testStaticImportReadWrite() throws Exception {
		performStaticImportTest("TestStaticImportReadWrite",
				"StaticImportReadWriteReference");
	}

	public void testStaticImportNone() throws Exception {
		performStaticImportTest("TestStaticImportNone",
				"StaticImportNoReference");
	}
	...
	/*
	 * disabled: no support for reusing existing getters/setters public void
	 * testThisExpressionInnerWithSetter() throws Exception {
	 * existingTest("field"); }
	 * 
