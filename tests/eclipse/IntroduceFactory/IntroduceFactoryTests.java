...
public class IntroduceFactoryTests extends TestCase {
	...
	/**
	 * Produces a test file name based on the name of this JUnit testcase. For
	 * input files, trims off the trailing part of the test name that begins
	 * with a '_', to get rid of the options part, so that we can have a single
	 * (suite of) input file(s) but several outputs dependent on the option
	 * settings.
	 * 
	 * @param input
	 *            true iff the requested file is an input file.
	 * @return the name of the test file, with a trailing "_in.java" if an input
	 *         file and a trailing "_XXX.java" if an output file and the test
	 *         name/options are "_XXX".
	 */
	private String getSimpleTestFileName(boolean input) {
		...
		String fileName = (input ? (testName.substring(4, endIdx) + "_in")
	...
	/**
	 * Produces a test file name based on the name of this JUnit testcase, like
	 * getSimpleTestFileName(), but also prepends the appropriate version of the
	 * resource path (depending on the value of <code>positive</code>). Test
	 * files are assumed to be located in the resources directory.
	 * 
	 * @param positive
	 *            true iff the requested file is for a positive unit test
	 * @param input
	 *            true iff the requested file is an input file
	...
	/**
	 * Produces a compilation unit from an input source file whose name is based
	 * on the testcase name. Test files are assumed to be located in the
	 * resources directory.
	 * 
	 * @param pack
	 * @param positive
	 ...
	 * @return the test file name
	 */
	private String getBugTestFileName(String project, String pack,
			String fileName, boolean input) {
		String testName = getName();
	...
	private CompilationUnit createCUForBugTestCase(String project, String pack,
		...
		for (Iterator<CompilationUnit> iter = prog.compilationUnitIterator(); iter
				.hasNext();) {
	...
	private CompilationUnit createCUForSimpleTest(boolean positive,
			boolean input) {
		String fileName = getTestFileName(positive, input);
		Program prog = CompileHelper.compile(fileName);
		CompilationUnit cu = null;
	...
	/**
	 * Finds and returns the selection markers in the given source string, i.e.
	 * the first occurrences of <code>SELECTION_START_HERALD</code> and
	...
	private FileRange findSelectionInSource(CompilationUnit cu) {
		...
		assertNotNull(begin);
		assertNotNull(end);
		return new FileRange(begin.filename, begin.el, begin.ec + 1, end.sl,
				end.sc);
	}

	private void doSingleUnitTest(boolean protectConstructor,
			CompilationUnit cu, String outputFileName) {
		Program out = CompileHelper.compile(outputFileName);
		assertNotNull(out);

		FileRange selection = findSelectionInSource(cu);
		ConstructorDecl cd;
		ClassInstanceExpr cie = PromoteTempToFieldTests.findNode(cu,
				ClassInstanceExpr.class, selection.sl, selection.sc,
				selection.el, selection.ec - 1);
		if (cie != null) {
			cd = cie.decl();
		} else {
			cd = PromoteTempToFieldTests.findNode(cu, ConstructorDecl.class,
					selection.sl, selection.sc, selection.el, selection.ec - 1);
			assertNotNull(cd);
		}
	...
	/**
	 * Tests the IntroduceFactoryRefactoring refactoring on a single input
	 * source file whose name is the test name (minus the "test" prefix and any
	 * trailing options indicator such as "_FFF"), and compares the transformed
	 * code to a source file whose name is the test name (minus the "test"
	 * prefix). Test files are assumed to be located in the resources directory.
	 * 
	 * @param protectConstructor
	 *            true iff IntroduceFactoryRefactoring should make the
	 *            constructor private
	 * @throws Exception
	 */
	 ...
	 * @throws Exception
	 */
	protected void singleUnitBugHelper(String baseFileName,
			boolean protectConstructor) throws Exception {
		CompilationUnit cu = createCUForBugTestCase(null, null, baseFileName,
				true);
		doSingleUnitTest(protectConstructor, cu,
				getBugTestFileName(null, null, baseFileName, false));
	}
	 ...
	 *            the name to use for the generated factory method
	 ...
	 * 
	 * @param fileName
	 *            the base name of the source file (minus the "_in" suffix)
	 * @param pack
	 *            an IPackageFragment for the containing package
	...
	private CompilationUnit[] createCUsFromBaseNames(String... baseNames) {
		...
		for (int i = 0; i < baseNames.length; ++i)
			fileNames[i] = "tests/eclipse/IntroduceFactory/positive/"
					+ baseNames[i] + "_in.java";
	...
	private CompilationUnit[] createCUsForBugTestCase(String... baseNames) {
		...
		String[] fileNames = new String[baseNames.length];
		for (int i = 0; i < baseNames.length; ++i)
			fileNames[i] = "tests/eclipse/IntroduceFactory/Bugzilla/"
					+ testNumber + "/" + baseNames[i] + ".java";
		Program prog = CompileHelper.compile(fileNames);
		CompilationUnit[] cus = new CompilationUnit[baseNames.length];
		for (Iterator<CompilationUnit> iter = prog.compilationUnitIterator(); iter
				.hasNext();) {
	...
	private void doMultiUnitTest(CompilationUnit[] CUs, String testPath,
			String[] outputFileBaseNames, String factoryClassName) {
		String testName = getName();
		...
		for (int i = 0; i < CUs.length; i++) {
			...
			String testOptions = (optIdx >= 0) ? testName.substring(optIdx)
					: "";
			outputFileNames[i] = testPath + outputFileBaseNames[i]
					+ testOptions + "_out.java";
		}
		Program out = CompileHelper.compile(outputFileNames);
		FileRange selection = findSelectionInSource(CUs[0]);
		ConstructorDecl cd;
		ClassInstanceExpr cie = PromoteTempToFieldTests.findNode(CUs[0],
				ClassInstanceExpr.class, selection.sl, selection.sc,
				selection.el, selection.ec - 1);
		if (cie != null) {
			cd = cie.decl();
		} else {
			cd = PromoteTempToFieldTests.findNode(CUs[0],
					ConstructorDecl.class, selection.sl, selection.sc,
					selection.el, selection.ec - 1);
			assertNotNull(cd);
		}
		...
		try {
			...
			if (factoryClassName != null) {
				...
				factory.hostType().doMoveMembers(
						Collections.singleton((MemberDecl) factory),
						factoryClass);
			}
	...
	/**
	 * Tests the IntroduceFactoryRefactoring refactoring on a set of input
	 * source files whose names are supplied in the <code>fileBaseNames</code>
	 * argument, and compares the transformed code to source files whose names
	 * are the input base names plus the options suffix (e.g. "_FFF"). Test
	 * files are assumed to be located in the resources directory.
	 * 
	 * @param staticFactoryMethod
	 *            true iff IntroduceFactoryRefactoring should make the factory
	 *            method static
	 * @param inputFileBaseNames
	 *            an array of input source file base names
	 * @throws Exception
	 */
	void multiUnitHelper(boolean staticFactoryMethod,
			String[] inputFileBaseNames) {
	 ...
	 *            an array of input source file base names
	 * @param factoryClassName
	 *            the fully-qualified name of the class to receive the factory
	 *            method, or null if the factory method is to be placed on the
	 *            class defining the given constructor
	 * @throws Exception
	 */
	void multiUnitBugHelper(boolean staticFactoryMethod,
			String[] inputFileBaseNames, String factoryClassName) {
		CompilationUnit[] cus = createCUsForBugTestCase(inputFileBaseNames);
		String testName = getName();
		String testNumber = testName.substring("test".length());
		String testPath = "tests/eclipse/IntroduceFactory/Bugzilla/"
				+ testNumber + "/";
	...
	// ================================================================================
	//
	static final String[] k_Names = { "createThing", "ThingFactory",
			"IThingFactory" };
	...
	//
	/*
	 * disabled: does not compile public void testCtorThrows() throws Exception
	 * { singleUnitHelper(true); }
	 */
	//
	// ================================================================================
	...
	/*
	 * disabled: no support for creating factory class public void
	 * testTypeParam2() throws Exception { namesHelper(null, "p.Factory"); }
	 */
	//
	// ================================================================================
	...
	public void testVarArgsCtor() throws Exception {
		// RMF - As of I20050202, search engine doesn't reliably find call sites
		// to varargs methods
		singleUnitHelper(true);
	}
	...
	public void testMultipleUnits_FFF() throws Exception {
		multiUnitHelper(false, new String[] { "MultiUnit1A", "MultiUnit1B",
				"MultiUnit1C" });
	}
	...
	//
	/*
	 * disabled: does not compile public void test45942() throws Exception {
	...
	/*
	 * disabled: tests idiosyncratic feature public void test59283() throws
	 * Exception { multiProjectBugHelper(new String[] { "proj1/pA/A",
	 * "proj2/pB/B" }, new String[] { "proj2:proj1" }); }
	 */
